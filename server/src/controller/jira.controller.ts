...
// export const trackIssueChange = async (c: Context) => {
//   try {
//     const accessToken = c.req.header("Authorization")?.replace("Bearer ", "");
//     const payload = await c.req.json();

//     // Verify webhook secret if you have one
//     const webhookSecret = c.req.header("X-Atlassian-Webhook-Secret");
...
//     const reporter = payload.issue?.fields?.reporter;

//     if (!issueKey || !cloudId) {
//       throw new BackendError("BAD_REQUEST", {
//         details: "Missing required issue information",
//       });
//     }
...
//     const reporterId = reporter?.accountId;
...
//       return c.json({
//         success: true,
//         message: "Not a status change, no points awarded",
//       });
//     }
...
//     if (
//       statusCategory === "Done" &&
//       statusChange.fromString !== statusChange.toString
//     ) {
//       pointsToAward = calculatePoints(payload.issue);
//       newTransition.pointsAwarded = pointsToAward;
//     }

//     await (await db()).collection("issueTracks").updateOne(
//       { issueKey },
//       {
//         $set: {
...
//           assigneeId,
//           reporterId,
//           projectId: payload.issue.fields.project.id,
//         },
//         $push: {
...
//     if (pointsToAward > 0 && assigneeId) {
//       await (await db()).collection("userPoints").updateOne(
//         { userId: assigneeId },
//         {
...
//               reason: `Completed issue ${issueKey}`,
//               timestamp: new Date(),
//             },
//           },
//         },
...
// export const getIssueHistory = async (c: Context) => {
//   const token = c.req.header("Authorization");
...
//   const cloudId = projectsCache?.sites[0]?.cloudId;

//   if (!cloudId) {
//     throw new BackendError("NOT_FOUND", {
//       details: "Cloud ID not found",
//     });
//   }

//   try {
//     const issueResponse = await fetch(
//       `https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issue/${issueKey}?expand=changelog,renderedFields`,
//       {
//         headers: {
//           Authorization: token,
//           Accept: "application/json",
...
//     if (!issueResponse.ok) {
//       throw new BackendError("INTERNAL_ERROR", {
//         details: "Failed to fetch issue details",
//       });
//     }
...
//     issueData.changelog.histories.forEach((history) => {
...
//         );
//         if (points !== 0) {
//           pointsActivities.push({
//             issueKey,
//             userId: history.author.accountId,
//             points,
//             reason: `Status change from ${statusChange.fromString} to ${statusChange.toString}`,
...
//       const points = calculateWorklogPoints(worklog.timeSpentSeconds);
//       if (points !== 0) {
//         pointsActivities.push({
//           issueKey,
//           userId: worklog.author.accountId,
//           points,
//           reason: `Logged work: ${worklog.timeSpent}`,
//           timestamp: worklog.started,
//           activity: {
//             type: "worklog",
//             details: `Time spent: ${worklog.timeSpent}`,
//           },
//         });
//       }
//     });

//     await updateUserPoints(pointsActivities);

//     const issueHistory: JiraIssueHistory = {
//       id: issueData.id,
//       key: issueData.key,
//       changelog: issueData.changelog.histories.map((history) => ({
//         id: history.id,
//         author: {
//           accountId: history.author.accountId,
//           displayName: history.author.displayName,
...
//         items: history.items,
//       })),
//       transitions: issueData.changelog.histories
//         .filter((history) =>
...
//         updated: comment.updated,
//       })),
//       worklog: worklogData.worklogs.map((worklog) => ({
//         id: worklog.id,
//         author: {
...
//   return Math.min(Math.floor(hours), 8);
// }

// async function updateUserPoints(activities: PointsActivity[]) {
//   const bulk = (await db())
...
export const getAudits = async (cloudId: string, accessToken: string) => {
  try {
    ...
    const [auditData, issueEvents] = await Promise.all([
      ...
      // commentsResponse.json() as Promise<{ issues: JiraIssue[] }>,
    ]);
    ...
    return {
      ...
      // comments: (comments?.issues || []).map((issue) => ({
      //   issueKey: issue.key,
      //   comments: issue.fields?.comment?.comments || [],
      // })),
    };
